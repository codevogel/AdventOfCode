...
class Program
{
    ...
    private static void Solve(IEnumerable<string> input)
    {
		...
		// Solve A:
		Console.WriteLine(directories.Select(directory => directory.FileSize).Where(fileSize => fileSize <= 100000).Sum());
		// Solve B:
		int availableSpace = 70000000 - directoryStack.Last().FileSize;
		int requiredSpace = 30000000;
		Console.WriteLine(directories.Select(directory => directory.FileSize).Where(fileSize => availableSpace + fileSize >= requiredSpace).Min());
    }
